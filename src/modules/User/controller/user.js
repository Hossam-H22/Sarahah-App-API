...
export const profilePic = asyncHandler(async (req, res, next) => {
    ...
    // uploade new photo
    const { secure_url, public_id } = await cloudinary.uploader.upload(req.file.path, { folder: `${process.env.APP_NAME}/user/${req.user._id}/profile` });
...
export const profileCovPic = asyncHandler(async (req, res, next) => {
    ...
    for(const file of req.files) {
        // uploade new photo
        const { secure_url, public_id } = await cloudinary.uploader.upload(file.path, { folder: `${process.env.APP_NAME}/user/${req.user._id}/profile/cover` });
